...
def pipeline_sentiment(url_value, api_key, model):
    ...
    text_tupple = [get_sentiment(i, model) for i in tqdm(result[config.col_text_comment])]
    result[[config.col_sentiment, "score"]] = pd.DataFrame(list(text_tupple))
    return result
